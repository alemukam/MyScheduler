...
@section('content')
    ...
    @if(Auth::user() -> id ==  $event -> group['moderator_id'])
        <a class="btn btn-outline-primary" href="{{ url('groups/'. $event['group_id'] .'/group-events/'. $event['id'] . '/edit') }}">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <small>{{ __('events/show.group') }}: "{{ $event -> group['name'] }}"</small>
                    </div>
                    <div class="modal-footer">
                        {!! Form::open(['action' => ['Event_GroupController@destroy', $event['group_id'], $event['id']], 'method' => 'POST']) !!}
                            ...
                            {!! Form::submit(__('events/show.btn_delete'), ['class' => 'btn btn-danger']) !!}
                        {!! Form::close() !!}
                    </div>
                </div>
            </div>
    ...
    <h3>{{ __('events/show.description') }}t</h3>
