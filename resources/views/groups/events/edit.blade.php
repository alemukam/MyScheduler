...
@section('content')
    ...
    <small>{{ __('events/create.group') }}: <strong>{{ $event -> group['name'] }}</strong></small>
    <hr>
    {!! Form::open(['action' => ['Event_GroupController@update', $event -> group['id'], $event['id']], 'method' => 'POST']) !!}
        ...
        <div class="form-group">
            ...
            {{ Form::textarea('description', $event['description'], ['required', 'type' => 'text', 'placeholder' => __('events/create.description_ph'), 'class' => 'form-control' . ($errors->has('description') ? ' is-invalid' : '')]) }}
        </div>
