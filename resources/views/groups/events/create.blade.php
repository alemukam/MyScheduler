...
@section('content')
    <h1>{{ __('events/create.header_g') }} "{{ $data['name'] }}"</h1>
    {!! Form::open(['action' => ['Event_GroupController@store', $data['id']], 'method' => 'POST']) !!}
        ...
        <div class="form-group">
            ...
            {{ Form::textarea('description', '', ['required', 'type' => 'text', 'placeholder' => __('events/create.description_ph'), 'class' => 'form-control' . ($errors->has('description') ? ' is-invalid' : '')]) }}
        </div>
